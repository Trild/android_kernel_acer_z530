...
static int Cam0I2cStatus = 0, Cam1I2cStatus = 0;
...
int iReadRegI2C(u8 *a_pSendData , u16 a_sizeSendData, u8 *a_pRecvData, u16 a_sizeRecvData, u16 i2cId)
{
    ...
    g_pstI2Cclient->addr = (i2cId >> 1);
    g_pstI2Cclient->ext_flag = (g_pstI2Cclient->ext_flag)&(~I2C_DMA_FLAG);
   PK_ERR("[CAMERA SENSOR] BUS1 I2C Addr = 0x%x\n", g_pstI2Cclient->addr);
    /* Remove i2c ack error log during search sensor */
    ...
    spin_lock(&kdsensor_drv_lock);
    g_pstI2Cclient2->addr = (i2cId >> 1);
    PK_ERR("[CAMERA SENSOR] BUS2 I2C Addr = 0x%x\n", g_pstI2Cclient2->addr);
    /* Remove i2c ack error log during search sensor */
...
MINT32 i = 0;
    ...
    if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
        ...
        /*  */
        ret = g_pInvokeSensorFunc[i]->SensorOpen(&sensor_id);
...
#endif
	if (DUAL_CAMERA_MAIN_SENSOR == g_invokeSocketIdx[i] && !Cam0I2cStatus)
		Cam0I2cStatus = 2;
	else if(DUAL_CAMERA_SUB_SENSOR == g_invokeSocketIdx[i] && !Cam1I2cStatus)
		Cam1I2cStatus = 2;
        PK_ERR("SensorOpen");
        return ret;
        }
	 ...
	 if (sensor_id == OV8858_SENSOR_ID)
		Cam0I2cStatus = 1;
	else if (sensor_id == OV8858S_SENSOR_ID)
		Cam1I2cStatus = 1;
	pr_err("%s Cam_%d sensorId:0x%x\n", __func__, g_invokeSocketIdx[i], sensor_id);
        /* set i2c slave ID */
...
static int cam0_i2c_status_read(struct seq_file *m, void *v)
{
   ...
   else if(2 == Cam0I2cStatus)
	strcpy(status, "failed");
   else
	strcpy(status, "unknown");

   seq_printf(m, "%s\n", status);
   PK_ERR("cam0_i2c_status_read %d\n", Cam0I2cStatus);
   return 0;
};
...
static  struct file_operations cam0_i2c_status_fops = {
    .owner = THIS_MODULE,
...
static int cam1_i2c_status_read(struct seq_file *m, void *v)
{
   ...
   if (1 == Cam1I2cStatus)
	strcpy(status, "success");
   else if(2 == Cam1I2cStatus)
	strcpy(status, "failed");
   else
	strcpy(status, "unknown");

   seq_printf(m, "%s\n", status);
   PK_ERR("cam1_i2c_status_read %d\n", Cam1I2cStatus);
   return 0;
};
...
static  struct file_operations cam1_i2c_status_fops = {
    .owner = THIS_MODULE,
