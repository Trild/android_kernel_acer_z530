...
u8 AFGroup_7021,AFInf_7022,AFMacro_7023,AFTune_7024;
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		LOG_INF("[Livet]i2c write id: 0x%x\n", imgsensor.i2c_write_id);
		spin_unlock(&imgsensor_drv_lock);
		do {
...
{
	int otp_flag, addr, temp, i,j,temp_test;
   ...
   for (j=0x7021; j<=0x7026; j++)
   {
       temp_test = 0;
       temp_test =read_cmos_sensor(j);
       LOG_INF("[Rear_ov8858_Sensor][read_otp_data] Address 0x%x data value is 0x%x\n", j, temp_test);
   }


	AFGroup_7021 =  read_cmos_sensor(0x7021);
	AFInf_7022 =  read_cmos_sensor(0x7022);
	AFMacro_7023 = read_cmos_sensor(0x7023);
	AFTune_7024 = read_cmos_sensor(0x7024);
	...
	if(addr != 0) {
		...
		LOG_INF("[Rear_ov8858_AF][read_otp_data] VCM_flag_7021 = 0x%x ---\n", (*otp_ptr).flag);
		LOG_INF("[Rear_ov8858_AF][read_otp_data] VCM_start_7022 = 0x%x ---\n", (* otp_ptr).VCM_start);
		LOG_INF("[Rear_ov8858_AF][read_otp_data] VCM_end_7023 = 0x%x ---\n", (* otp_ptr).VCM_end);
		LOG_INF("[Rear_ov8858_AF][read_otp_data] VCM_dir_7024 = 0x%x ---\n", (* otp_ptr).VCM_dir);
	}
	else {
		(* otp_ptr).VCM_start = 0;
		(* otp_ptr).VCM_end = 0;
...
static int apply_otp(struct otp_struct *otp_ptr)
{
	...
	int RG_Ratio_Typical = 0x127, BG_Ratio_Typical = 0x135;
...
for (k=0x5032; k<=0x5037; k++)
{
       ...
       LOG_INF("[ov8858][AWB_data] Address 0x%x data value is 0x%x\n", k, temp_test_k);
}
...
for (m=0x5800; m<=0x5803; m++)
{
       ...
       LOG_INF("[ov8858][LSC_data] Address 0x%x data value is 0x%x\n", m, temp_test_m);
}
	return (*otp_ptr).flag;
}
