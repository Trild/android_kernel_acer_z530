...
OV8858S_VERSION ov8858sversion = OV8858SR2A;

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV8858S_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static void r1asensor_init(void)
{
	LOG_INF("E\n");
//   ;-----------------------------------------
//   @@ OV8858SR1A Key 1632x1224_30FPS_MIPI_2LANE(Binning)
...
{
	LOG_INF("E\n");
//  @@ OV8858SR2A Initial 1632x1224_30FPS_MIPI_2LANE(Binning)
//  100 99 1632 1224 ; Resolution
//  102 80 1
//  102 3601 bb8;30fps
//  102 40 0 ; HDR Mode off
...
//  ;;
	...
	if (ov8858sversion == OV8858SR1A){
		LOG_INF("inter r1asensor_init() ov8858sversion = %d(0=r2a,1=r1a)\n",ov8858sversion); 
		r1asensor_init();
	}
...
{
	LOG_INF("E\n");
//  @@ OV8858SR2A VGA 120fps_PCLK=148.8M
//  100 99 640 480
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		LOG_INF("Livet get_imgsensor_id: 0x%x\n", imgsensor.i2c_write_id);
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if ((*sensor_id +1)== imgsensor_info.sensor_id) {
				if((read_cmos_sensor(0x302A)) == 0XB2){
 				ov8858sversion = OV8858SR2A;
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8858sversion = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8858sversion);	
				return ERROR_NONE;
				}
				else if((read_cmos_sensor(0x302A)) == 0XB1){
 				ov8858sversion = OV8858SR1A;
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8858sversion = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8858sversion);	
				return ERROR_NONE;
				}
				else{
				LOG_INF("read ov8858s R1A R2A bate fail\n");	  
...
static int apply_otp(struct otp_struct *otp_ptr)
{
	...
	int RG_Ratio_Typical = 0x13F, BG_Ratio_Typical = 0x11F;
	int k, temp_test_k,m, temp_test_m;;
...
for (k=0x5032; k<=0x5037; k++)
{
       temp_test_k = 0;
       temp_test_k =read_cmos_sensor(k);
       LOG_INF("[ov8858s][AWB_data] Address 0x%x data value is 0x%x\n", k, temp_test_k);
}		
...
for (m=0x5800; m<=0x5803; m++)
{
       temp_test_m = 0;
       temp_test_m =read_cmos_sensor(m);
       LOG_INF("[ov8858s][LSC_data] Address 0x%x data value is 0x%x\n", m, temp_test_m);
}	
	return (*otp_ptr).flag;
}
...
*
*************************************************************************/
static kal_uint32 open(UINT32 *camera_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		LOG_INF("open: Read sensor id: 0x%x\n", imgsensor.i2c_write_id);
		spin_unlock(&imgsensor_drv_lock);
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if ((sensor_id+1) == imgsensor_info.sensor_id) {				
...
UINT32 OV8858SR2A_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
