...
static ssize_t show_blue_led_switch(struct device *dev, struct device_attribute *attr, char *buf)
{
	int status = 0;

	status = mt_get_gpio_out(GPIO93);
	LEDS_DRV_DEBUG("[LED]get blue_led_switch value is:%d\n", status);

	return sprintf(buf, "%d\n", status);
}

static ssize_t store_blue_led_switch(struct device *dev, struct device_attribute *attr, const char *buf,
			  size_t size)
{
	char *pvalue = NULL;
	int enable = 0;
	...
	LEDS_DRV_DEBUG("set blue_led_switch enable:%d\n", enable);
	if (enable)
		mt_set_gpio_out(GPIO93, GPIO_OUT_ZERO);
	else
		mt_set_gpio_out(GPIO93, GPIO_OUT_ONE);

	return size;
}

static DEVICE_ATTR(blue_led, 0664, show_blue_led_switch, store_blue_led_switch);
...
static int __init mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	/* Disable charge_blue Led */
	mt_set_gpio_mode(GPIO93, GPIO_MODE_00);
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		}else if (strcmp(g_leds_data[i]->cdev.name, "gpio_switch") == 0) {
			rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_blue_led);
			if (rc) {
				LEDS_DRV_DEBUG("[LED]device_create_file blue_led fail!\n");
			}
		}
