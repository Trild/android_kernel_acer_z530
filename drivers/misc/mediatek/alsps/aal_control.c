...
int aal_use = 0;
int Acer_get_AAL_ALS_Data = 0; /*Acer 20150610 add to get Android 5.1 ALS data*/
		...
		switch (cmd)
		{
			...
			case AAL_GET_ALS_DATA: 
	            if ((dat = alsps_aal_get_data()) < 0){
					//For performance improvement, remove unused log
					//AAL_LOG("alsps_aal_get_data fail\n");
					dat = hwmsen_aal_get_data();
	            }
				
				AAL_LOG("Get als dat :%d\n", dat);
				Acer_get_AAL_ALS_Data = dat; /*Acer 20150610 add to get Android 5.1 ALS data*/
