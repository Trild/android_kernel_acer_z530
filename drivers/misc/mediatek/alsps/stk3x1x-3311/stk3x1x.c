...
/*Acer 20150610 for color engine debug*/
bool sunlight_content_color_engine_enable = false; //acer color function default off
...
#define APS_DBG(fmt, args...)    printk(KERN_ALERT fmt, ##args)                 
...
//defide LUX_OUT_MODE is not table output. //ken 20150709
#define LUX_OUT_MODE
...
struct stk3x1x_threshold {
	...
	unsigned  int result_ps_60mm_cover;
	unsigned  int result_als_value;
	unsigned  int als_transmittance_cali; // add for FTM ALS CALI
	unsigned  int als_adc_cali;// add for FTM ALS CALI
	unsigned  int ps_high_thd;
	unsigned  int ps_low_thd;
};
//20150602 Global Value
unsigned int cci_ps_high_thd = 1650;
unsigned int cci_ps_high_tmp_thd;
unsigned int cci_ps_low_thd = 1600;
unsigned int cci_ps_cover;
unsigned int cci_ps_60mm_cover;
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	int ret, diff;
	int ps_range = 700;
	int over_ps_range = 2300;
	int over_def_ct = 2000;
	...
	if(diff > obj->stk_max_min_diff)
	{
		...
		{
			if( (obj->psi > over_ps_range) && (obj->psi > (cci_ps_high_thd + ps_range) )){//over Value status
                                atomic_set(&obj->ps_high_thd_val, (over_def_ct + obj->stk_ht_n_ct)); 
				atomic_set(&obj->ps_low_thd_val , (over_def_ct + obj->stk_lt_n_ct)); 
				APS_LOG("%s: overVal change THD to boot HT=%d, LT=%d\n", __func__, atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val));
			}else if( atomic_read(&obj->ps_high_thd_val) > (cci_ps_high_thd + ps_range) ) {//normal Value status
				atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot); 
				atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot); 
				APS_LOG("%s: defaultVal change THD to boot HT=%d, LT=%d\n", __func__, atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val));
			}
...
int stk3x1x_cci_als_cali(int sSampleNo)
{
	...
	if(als_reading <= 154 || als_reading >= 1684){
...
static uint32_t stk3x1x_get_ps_reading_AVG(int sSampleNo){
	...
	while(DataCount < sSampleNo)
	{
		...
		if(err =stk3x1x_read_ps(stk3x1x_obj->client, &stk3x1x_obj->ps)){
			APS_ERR("[#23] stk3x1x_read_ps fail\n");
		}
		else{
			PSData = stk3x1x_obj->ps;
			APS_ERR("[#23] ps code = %d\n", PSData);
			sAvePsData +=  PSData;
			DataCount++;
		}
	}
...
/*----------------------------------------------------------------------------*///add Ken
static ssize_t stk3x1x_show_nvram_send(struct device_driver *ddri, char *buf)
{
    return 0;
}
/*----------------------------------------------------------------------------*///add Ken
static ssize_t stk3x1x_store_nvram_send(struct device_driver *ddri, const char *buf, size_t count)
{
	struct i2c_client *client;
	client = stk3x1x_i2c_client;
	...
	int NVR_result_ct, NVR_result_ps_cover, NVR_result_als_value ;
	int NVR_als_transmittance_cali, NVR_als_adc_cali ,NVR_ps_60mm_cover ;
	int NVR_ps_high_thd, NVR_ps_low_thd;
	...
	else if(8 != sscanf(buf, "%d %d %d %d %d %d %d %d", &NVR_result_ct , &NVR_result_ps_cover, &NVR_result_als_value , 
				&NVR_als_transmittance_cali , &NVR_als_adc_cali , &NVR_ps_60mm_cover, 
				&NVR_ps_high_thd , &NVR_ps_low_thd))
	{
		APS_ERR("invalid format: '%s'\n", buf);
		return 0;
	}

	APS_ERR("NVR_result_ct = %d\n", NVR_result_ct);
	APS_ERR("NVR_result_ps_cover = %d\n", NVR_result_ps_cover);
	APS_ERR("NVR_result_als_value = %d\n", NVR_result_als_value);
	APS_ERR("NVR_als_transmittance_cali = %d\n", NVR_als_transmittance_cali);
	APS_ERR("NVR_als_adc_cali = %d\n", NVR_als_adc_cali);
	APS_ERR("NVR_ps_60mm_cover = %d\n", NVR_ps_60mm_cover );
	APS_ERR("NVR_ps_high_thd = %d\n", NVR_ps_high_thd );
	APS_ERR("NVR_ps_low_thd = %d\n", NVR_ps_low_thd );

	if(NVR_ps_high_thd == 0)
		cci_ps_high_thd = 1650;
	else
		cci_ps_high_thd = NVR_ps_high_thd;
	if(NVR_ps_low_thd == 0)
		cci_ps_low_thd = 1600;
	else
		cci_ps_low_thd  = NVR_ps_low_thd;
	cci_result_ct   = NVR_result_ct;	
	cci_ps_cover    = NVR_result_ps_cover; //35mm
	cci_als_value   = NVR_result_als_value;  // 300 lux adc 
	cci_transmittance_cali = NVR_als_transmittance_cali;
	cci_als_adc_cali = NVR_als_adc_cali ; // 500 lux adc 
	cci_ps_60mm_cover = NVR_ps_60mm_cover; //50mm
	...
	APS_ERR("[#23]cci_ps_60mm_cover =  %d\n", cci_ps_60mm_cover);
...
static ssize_t stk3x1x_ps_color_store(struct device_driver *ddri, const char *buf, size_t count)
{
    int value = 0;
	int ret;
	...
	if (value == 1)
	{
		sunlight_content_color_engine_enable = true;
		APS_ERR("sunlight_content_color_engine_enable======%d\n", sunlight_content_color_engine_enable);
	}
	else if (value == 0)
	{
		sunlight_content_color_engine_enable = false;
		APS_ERR("sunlight_content_color_engine_enable======%d\n", sunlight_content_color_engine_enable);
	}
	return count;
}

static ssize_t stk3x1x_ps_color_show(struct device_driver *ddri, char *buf)
{
	if(!stk3x1x_obj)
	{
		APS_ERR("stk3x1x_obj is null!!\n");
...
//PS
//15mm 20150602
static ssize_t stk3x1x_show_BC_40mm(struct device_driver *ddri, char *buf)
{
	//
	//PS cal Gray card 4.0 cm
	uint32_t reading;
	int err;
	if(!stk3x1x_obj)
	{
	...
	APS_ERR("[#23]%s:Start calibrating with Gray Card 4.0 cm...\n", __func__);
	msleep(150);
	reading = stk3x1x_get_ps_reading_AVG(5);
	
	//cci_ps_high_thd = reading; //*****************This is F2N threshold***************
	cci_ps_high_tmp_thd = reading; //*****************This is F2N threshold***************

	APS_ERR("[#23]%s:calibrating with Gary Card 4.0 cm done!!!!! cci_ps_high_tmp_thd = %d\n", __func__, cci_ps_high_thd);
	return scnprintf(buf, PAGE_SIZE, "cci_ps_high_tmp_thd = %d\n", cci_ps_high_tmp_thd);
	//
}
...
static ssize_t stk3x1x_show_BC_ct(struct device_driver *ddri, char *buf)
{
	...
	cci_ps_high_thd = (((cci_ps_high_tmp_thd - cci_result_ct)*12)/10)+cci_result_ct;

	if ((cci_result_ct < 1500) && (cci_ps_high_thd > (cci_result_ct*1))) { // STK modify for LY28 BC 0cm 20150128
		...
		if(Diff_ThdH_CT > 60){
			cci_ps_low_thd = (((cci_ps_high_thd - cci_result_ct)*66)/100)+cci_result_ct; //STK modify for LY28 BC 0cm 20150128
			atomic_set(&stk3x1x_obj->ps_high_thd_val, cci_ps_high_thd);
			atomic_set(&stk3x1x_obj->ps_low_thd_val, cci_ps_low_thd);
			if((err = stk3x1x_write_ps_high_thd(stk3x1x_obj->client, atomic_read(&stk3x1x_obj->ps_high_thd_val)))) {
				APS_ERR("[#23]write high thd error: %d\n", err);
			...
			APS_ERR("[#23] ps_CCI_cali_BC_ct_show() Diff_ThdH_CT = %d\n", Diff_ThdH_CT);
		}
	} else {
		cci_ps_low_thd = 0;
		APS_ERR("[#23]---------------Test PS Gray card CT fail----------------\n");
	}
	APS_ERR("[#23] ps_CCI_cali_BC_ct_show() cci_ps_high_thd = %d, cci_ps_low_thd = %d, cci_result_ct=%d\n", cci_ps_high_thd, cci_ps_low_thd, cci_result_ct);
	
	return scnprintf(buf, PAGE_SIZE, "cci_result_ct = %d, cci_ps_low_thd = %d , cci_ps_high_thd = %d\n", cci_result_ct, cci_ps_low_thd,cci_ps_high_thd);
}

static ssize_t stk3x1x_show_test_35mm(struct device_driver *ddri, char *buf)
{
	...
	//get ps code for 5 times
	APS_ERR("%s:[#23]Start testing 35mm...\n", __func__);
	reading = stk3x1x_get_ps_reading_AVG(5);
	
	test_ps_code = reading;
	
	APS_ERR("[#23]ps_CCI_test_35mm() CCI_FTM_INTERRUPT = %d\n", CCI_FTM_INTERRUPT);
	APS_ERR("[#23] adc= %d\n",test_ps_code);
	if(test_ps_code >= cci_ps_high_thd){
		diff_test = test_ps_code - cci_result_ct;
		cci_ps_cover = test_ps_code;
		if((diff_test<800)&&(diff_test>40)){
		}else{
			return scnprintf(buf, PAGE_SIZE, "Test PS Gray card 3.5 cm FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_cover);
		}
		ps_test_cover = true;
		if(CCI_FTM_INTERRUPT != 0){
			APS_ERR("[#23]Test PS Gray card 3.5 cm SUCCESS: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
			return scnprintf(buf, PAGE_SIZE, "Test PS Gray card 3.5 cm SUCCESS: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
		} else {
			ps_test_cover = false;
			APS_ERR("[#23]Test PS Gray card 3.5 cm FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_cover);
			return scnprintf(buf, PAGE_SIZE, "Test PS Gray card 3.5 cm FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_cover);
		}
	} else {
		ps_test_cover = false;
		...
		if(CCI_FTM_INTERRUPT != 0){
			APS_ERR("[#23]Test PS Gray card 3.5 cm FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
			return scnprintf(buf, PAGE_SIZE, "Test PS Gray card 3.5 cm FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
		} else {
			APS_ERR("[#23]Test PS Gray card 3.5 cm FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_cover);
			return scnprintf(buf, PAGE_SIZE, "Test PS Gray card 3.5 cm FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_cover);
		}
...
static ssize_t stk3x1x_show_test_50mm(struct device_driver *ddri, char *buf)
{
	//Test black card cover
	uint32_t reading;
	uint32_t test_ps_code;
	...
	//get ps code for 5 times
	APS_ERR("%s:[#23]Start testing 50mm...\n", __func__);
	reading = stk3x1x_get_ps_reading_AVG(5);
	
	test_ps_code = reading;
	
	APS_ERR("[#23]ps_CCI_test_50mm() CCI_FTM_INTERRUPT = %d\n", CCI_FTM_INTERRUPT);
	APS_ERR("[#23] adc= %d\n",test_ps_code);
	if(test_ps_code <= cci_ps_low_thd){
		diff_test = test_ps_code - cci_result_ct;
		cci_ps_60mm_cover = test_ps_code;
		if((diff_test<400)&&(diff_test>10)){
		}else{
			return scnprintf(buf, PAGE_SIZE, "Test PS Gray card 5.0 cm FAIL: ps_test_cover = %d, g_result_ps_60mm_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_60mm_cover);
		}
		ps_test_cover = true;
		if(CCI_FTM_INTERRUPT != 0){
			APS_ERR("[#23]Test PS Gray card 5.0 cm SUCCESS: ps_test_cover = %d, g_result_ps_60mm_cover = %d, INT PASS\n", ps_test_cover, cci_ps_60mm_cover);
			return scnprintf(buf, PAGE_SIZE, "Test PS Gray card 5.0 cm SUCCESS: ps_test_cover = %d, g_result_ps_60mm_cover = %d, INT PASS\n", ps_test_cover, cci_ps_60mm_cover);
		} else {
			ps_test_cover = false;
			APS_ERR("[#23]Test PS Gray card 5.0 cm FAIL: ps_test_cover = %d, g_result_ps_60mm_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_60mm_cover);
			return scnprintf(buf, PAGE_SIZE, "Test PS Gray card 5.0 cm FAIL: ps_test_cover = %d, g_result_ps_60mm_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_60mm_cover);
		}
	} else {
		ps_test_cover = false;
		...
		if(CCI_FTM_INTERRUPT != 0){
			APS_ERR("[#23]Test PS Gray card 5.0 cm FAIL: ps_test_cover = %d, g_result_ps_60mm_cover = %d, INT PASS\n", ps_test_cover, cci_ps_60mm_cover);
			return scnprintf(buf, PAGE_SIZE, "Test PS Gray card 5.0 cm FAIL: ps_test_cover = %d, g_result_ps_60mm_cover = %d, INT PASS\n", ps_test_cover, cci_ps_60mm_cover);
		} else {
			APS_ERR("[#23]Test PS Gray card 5.0 cm FAIL: ps_test_cover = %d, g_result_ps_60mm_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_60mm_cover);
			return scnprintf(buf, PAGE_SIZE, "Test PS Gray card 5.0 cm FAIL: ps_test_cover = %d, g_result_ps_60mm_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_60mm_cover);
		}
...
static ssize_t stk3x1x_show_test_Light(struct device_driver *ddri, char *buf)
{
	...
	als_lux_reading = stk3x1x_get_als_value(obj,als_reading);
	APS_ERR("[#23]%s: als_lux_reading = %d\n", __func__, als_lux_reading);
	cci_als_value = als_reading; //return raw data only
	if(cci_als_adc_cali > cci_als_value){
		if((cci_als_value>132)&&(cci_als_value<845)){
			APS_ERR("[#23]%s: Start testing light done!!! cci_als_value = %d \n", __func__, cci_als_value);
			return scnprintf(buf, PAGE_SIZE, "PASS:cci_als_value = %d\n", cci_als_value);	
		}else{
			APS_ERR("[#23]%s: Start testing light done!!! cci_als_value = %d \n", __func__, cci_als_value);
			return scnprintf(buf, PAGE_SIZE, "FAIL:cci_als_value = %d\n", cci_als_value);
		}
	}else{
			APS_ERR("[#23]%s: Start testing light done!!! cci_als_value = %d \n", __func__, cci_als_value);
			return scnprintf(buf, PAGE_SIZE, "FAIL:cci_als_value = %d\n", cci_als_value);
...
static ssize_t als_CCI_cali_Light_show(struct device_driver *ddri, char *buf)
{
	...
	if(ret > 0){
		...
		return scnprintf(buf, PAGE_SIZE, "PASS!!! cci_transmittance_cali = %d, cci_als_adc_cali = %d\n", cci_transmittance_cali,cci_als_adc_cali);
	} else {
...
static ssize_t stk3x1x_show_nvdata(struct device_driver *ddri, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "cci_ps_high_thd = %d, cci_ps_low_thd = %d, cci_result_ct = %d, cci_ps_cover = %d, cci_als_value = %d, cci_ps_60mm_cover = %d\n", cci_ps_high_thd, cci_ps_low_thd, cci_result_ct, cci_ps_cover, cci_als_value, cci_ps_60mm_cover);		
}
...
#endif
static DRIVER_ATTR(color,    S_IWUSR | S_IRUGO, stk3x1x_ps_color_show,  stk3x1x_ps_color_store);
#ifdef STK_ALS_FIR
static DRIVER_ATTR(firlen,    S_IWUSR | S_IRUGO, stk3x1x_show_firlen,  stk3x1x_store_firlen);
#endif
//20150602 
static DRIVER_ATTR(BC_40mm,    S_IWUSR | S_IRUGO, stk3x1x_show_BC_40mm,  NULL);
static DRIVER_ATTR(BC_ct,    S_IWUSR | S_IRUGO, stk3x1x_show_BC_ct,  NULL);
static DRIVER_ATTR(test_35mm,    S_IWUSR | S_IRUGO, stk3x1x_show_test_35mm,  NULL);
static DRIVER_ATTR(test_50mm,    S_IWUSR | S_IRUGO, stk3x1x_show_test_50mm,  NULL);
...
//20150602
static DRIVER_ATTR(nvram_send,    S_IWUSR | S_IRUGO, stk3x1x_show_nvram_send,  stk3x1x_store_nvram_send);
...
//    &driver_attr_i2c,
    ...
    &driver_attr_BC_40mm,
    &driver_attr_BC_ct,
    &driver_attr_test_35mm,
    &driver_attr_test_50mm,
    &driver_attr_test_Light,
    &driver_attr_cali_Light,
    &driver_attr_nvdata,
    &driver_attr_nvram_send,
};
...
{
	int lux;
	lux = als*cci_transmittance_cali/1100;
...
uint32_t stk_alscode2lux(struct stk3x1x_data *ps_data, uint32_t alscode)
{
	...
	alscode/=cci_transmittance_cali;
	return alscode;
} 
*/
/*----------------------------------------------------------------------------*/
...
#endif
{
	...
	switch (cmd)
	{
		...
		case ALSPS_IOCTL_SET_CALI_CCI:
			...
			cci_ps_60mm_cover = calibration_threshold.result_ps_60mm_cover;
			APS_ERR("%s ALSPS_IOCTL_SET_CALI_CCI cci_result_ct = %d, cci_ps_cover = %d, cci_als_value = %d, cci_ps_60mm_cover = %d\n", __FUNCTION__, cci_result_ct, cci_ps_cover, cci_als_value, cci_ps_60mm_cover);
			...
			APS_ERR("%s ALSPS_IOCTL_SET_CALI_CCI als_transmittance_cali = %d, als_adc_cali = %d\n", __FUNCTION__, cci_transmittance_cali, cci_als_adc_cali);
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	stk3x1x_init_flag = 0;
#endif
	if((cci_ps_high_thd == 1650) && (cci_ps_low_thd = 1600)){
		atomic_set(&obj->ps_high_thd_val, cci_ps_high_thd); 
		atomic_set(&obj->ps_low_thd_val, cci_ps_low_thd); 
