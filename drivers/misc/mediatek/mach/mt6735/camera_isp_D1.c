...
EXIT:
	...
	if (Ret < 0 )
		...
		cam_isp_status = 1;
	pr_err("%s ---ret:%d, isp status:%d\n", __func__, Ret, cam_isp_status);
	return Ret;
...
static int cam_isp_status_read(struct seq_file *m, void *v)
{
   ...
   seq_printf(m, "%s\n", status);
   pr_err("Got cam_isp_status_read %s\n", status);
   return 0;
};
...
static  struct file_operations cam_isp_status_fops = {
    .owner = THIS_MODULE,
