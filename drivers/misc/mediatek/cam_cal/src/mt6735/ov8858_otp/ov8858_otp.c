...
extern  u8 AFGroup_7021,AFInf_7022,AFMacro_7023,AFTune_7024;
...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "ov8858_CAM_CAL_DRV"
...
int selective_read_region(u32 addr, BYTE* data,u16 i2c_id,u32 size)
{
  //  u32 page = addr/PAGE_SIZE; /* size of page was 256 */
	...
	*buff=AFGroup_7021;
	CAM_CALDB("AF_buff_7021 = 0x%x---\n", *buff);
	buff+=1;

	*buff=AFInf_7022;
	CAM_CALDB("AF_buff_7022 = 0x%x---\n", *buff);	
	buff+=1;
        
	*buff=AFMacro_7023;
	CAM_CALDB("AF_buff_7023 = 0x%x---\n", *buff);	
	buff+=1;

	*buff=AFTune_7024;
	CAM_CALDB("AF_buff_7024 = 0x%x---\n", *buff);

	CAM_CALDB(" addr =%x-- size %d-- data read = %d\n", addr,size, ret);
    return ret;
}
...
static int ov8858_CAM_CAL_Ioctl(struct inode * a_pstInode,
struct file * a_pstFile,
unsigned int a_u4Command,
unsigned long a_u4Param)
#else
static long ov8858_CAM_CAL_Ioctl(
    struct file *file,
    unsigned int a_u4Command,
    unsigned long a_u4Param
)
...
static const struct file_operations g_stCAM_CAL_fops =
{
    ...
    //.ioctl = ov8858_CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
    .compat_ioctl = ov8858_Ioctl_Compat,
#endif
    .unlocked_ioctl = ov8858_CAM_CAL_Ioctl
};
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    CAM_CAL_class = class_create(THIS_MODULE, "ov8858_CAM_CALdrv");
