	...
	case DISP_IOCTL_AAL_GET_HIST:
		{
			...
			acer_get_display_condition(cmdq); /*20150511 Acer add for color engine*/
			break;
		}
...
/*Acer 20150610 add for color engine*/
void acer_get_display_condition(void *cmdq)
{
    if (sunlight_content_color_engine_enable) {
        g_acer_smart_color.acer_content_aware_profile = acer_color_analysis_histogram(g_aal_hist.maxHist, AAL_HIST_BIN);
        g_acer_smart_color.acer_auto_backlight_enable = Acer_get_AAL_ALS_Data;
        g_acer_smart_color.acer_sunlight_readable_profile = acer_ALS_analysis_function(g_acer_smart_color.acer_auto_backlight_enable/*ALS_data_acer_sunlight_readable*/);
        g_acer_smart_color.acer_sunlight_content_profile = acer_sunlight_content_analysis_function(g_acer_smart_color.acer_content_aware_profile,g_acer_smart_color.acer_sunlight_readable_profile );
        set_acer_color_profile(cmdq);
    }
    else if ( g_acer_smart_color.acer_previous_color_sunlight_engine_enable && (!sunlight_content_color_engine_enable) ) {
        acer_color_engine_set_saturation(0, cmdq);
        acer_color_initial();
        g_acer_smart_color.acer_previous_sunlight_content_profile = -1;
        g_acer_smart_color.acer_combine_Content_PreviousFlag = -1;
        g_acer_smart_color.acer_combine_Sunlight_PreviousFlag = -1;
        acer_color_engine_set_luma(cmdq,-1,-1);
    }
    g_acer_smart_color.acer_previous_color_sunlight_engine_enable = sunlight_content_color_engine_enable;
}

/*Acer 201500610 add for color engine*/
...
{
    int i;
    g_acer_smart_color.acer_final_luma_curve = Sunlight_Content_acer_Luma_Gain[sunlight_flag][content_flag];

    if ( sunlight_flag == -1 && content_flag == -1 ) {
        for (i=0;i<29;i++) {
            g_acer_smart_color.acer_combine_luma_gain[i] = 256;
        }
    }
    else {
        for (i=0;i<29;i++) {
            g_acer_smart_color.acer_combine_luma_gain[i] = ( g_acer_smart_color.acer_final_luma_curve[i+1] * 256 ) / ( (i+1) * 32 ) ;
            if (g_acer_smart_color.acer_combine_luma_gain[i] < 256 )
                g_acer_smart_color.acer_combine_luma_gain[i] = 256;
            else if (g_acer_smart_color.acer_combine_luma_gain[i] > 1023)
                g_acer_smart_color.acer_combine_luma_gain[i] = 1023;
        }
    }
    DISP_REG_MASK(cmdq, DISP_AAL_DRE_FLT_FORCE(0), DRE_REG_2(g_acer_smart_color.acer_combine_luma_gain[0], 0, g_acer_smart_color.acer_combine_luma_gain[1], 12), ~0);
    DISP_REG_MASK(cmdq, DISP_AAL_DRE_FLT_FORCE(1), DRE_REG_2(g_acer_smart_color.acer_combine_luma_gain[2], 0, g_acer_smart_color.acer_combine_luma_gain[3], 12), ~0);
    DISP_REG_MASK(cmdq, DISP_AAL_DRE_FLT_FORCE(2), DRE_REG_2(g_acer_smart_color.acer_combine_luma_gain[4], 0, g_acer_smart_color.acer_combine_luma_gain[5], 11), ~0);
    DISP_REG_MASK(cmdq, DISP_AAL_DRE_FLT_FORCE(3), DRE_REG_3(g_acer_smart_color.acer_combine_luma_gain[6], 0, g_acer_smart_color.acer_combine_luma_gain[7], 11, g_acer_smart_color.acer_combine_luma_gain[8], 21), ~0);
    DISP_REG_MASK(cmdq, DISP_AAL_DRE_FLT_FORCE(4), DRE_REG_3(g_acer_smart_color.acer_combine_luma_gain[9], 0, g_acer_smart_color.acer_combine_luma_gain[10], 10, g_acer_smart_color.acer_combine_luma_gain[11], 20), ~0);
    DISP_REG_MASK(cmdq, DISP_AAL_DRE_FLT_FORCE(5), DRE_REG_3(g_acer_smart_color.acer_combine_luma_gain[12], 0, g_acer_smart_color.acer_combine_luma_gain[13], 10, g_acer_smart_color.acer_combine_luma_gain[14], 20), ~0);
    DISP_REG_MASK(cmdq, DISP_AAL_DRE_FLT_FORCE(6), DRE_REG_3(g_acer_smart_color.acer_combine_luma_gain[15], 0, g_acer_smart_color.acer_combine_luma_gain[16], 10, g_acer_smart_color.acer_combine_luma_gain[17], 20), ~0);
    DISP_REG_MASK(cmdq, DISP_AAL_DRE_FLT_FORCE(7), DRE_REG_3(g_acer_smart_color.acer_combine_luma_gain[18], 0, g_acer_smart_color.acer_combine_luma_gain[19], 9, g_acer_smart_color.acer_combine_luma_gain[20], 18), ~0);
    DISP_REG_MASK(cmdq, DISP_AAL_DRE_FLT_FORCE(8), DRE_REG_3(g_acer_smart_color.acer_combine_luma_gain[21], 0, g_acer_smart_color.acer_combine_luma_gain[22], 9, g_acer_smart_color.acer_combine_luma_gain[23], 18), ~0);
    DISP_REG_MASK(cmdq, DISP_AAL_DRE_FLT_FORCE(9), DRE_REG_3(g_acer_smart_color.acer_combine_luma_gain[24], 0, g_acer_smart_color.acer_combine_luma_gain[25], 9, g_acer_smart_color.acer_combine_luma_gain[26], 18), ~0);
    DISP_REG_MASK(cmdq, DISP_AAL_DRE_FLT_FORCE(10), DRE_REG_2(g_acer_smart_color.acer_combine_luma_gain[27], 0, g_acer_smart_color.acer_combine_luma_gain[28], 9), ~0);
}

/*Acer 20150610 add for color engine*/
void set_acer_color_profile(void *cmdq)
{
    if (g_acer_smart_color.acer_previous_sunlight_content_profile  != g_acer_smart_color.acer_sunlight_content_profile) {
        acer_color_engine_set_saturation(g_acer_smart_color.acer_sunlight_content_profile, cmdq);
    }
    g_acer_smart_color.acer_previous_sunlight_content_profile = g_acer_smart_color.acer_sunlight_content_profile;

    acer_sunlight_content_smooth(&g_acer_smart_color.acer_combine_Sunlight_CurrentFlag, &g_acer_smart_color.acer_combine_Content_CurrentFlag);

    if ( (g_acer_smart_color.acer_combine_Sunlight_CurrentFlag != g_acer_smart_color.acer_combine_Sunlight_PreviousFlag) || (g_acer_smart_color.acer_combine_Content_CurrentFlag != g_acer_smart_color.acer_combine_Content_PreviousFlag) ) {
        acer_color_engine_set_luma(cmdq,g_acer_smart_color.acer_combine_Sunlight_CurrentFlag,g_acer_smart_color.acer_combine_Content_CurrentFlag);
    }
    g_acer_smart_color.acer_combine_Sunlight_PreviousFlag = g_acer_smart_color.acer_combine_Sunlight_CurrentFlag;
    g_acer_smart_color.acer_combine_Content_PreviousFlag = g_acer_smart_color.acer_combine_Content_CurrentFlag;
}
