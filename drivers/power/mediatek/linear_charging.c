...
extern kal_int32 cci_ntc_fixed_25degc;
 /* ============================================================ // */
 /* extern function */
 /* ============================================================ // */
...
#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
			g_jeita_recharging_voltage = 4250;
#else
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (get_usb_current_unlimited()) {
			...
			if (g_bcct_flag == 1 && cci_ntc_fixed_25degc == 0) {
				/*cci_ntc_fixed_25degc flag for monkey test that need to fix charging current in high current. So, turn off bcct when monkey set this flag*/
...
PMU_STATUS BAT_TopOffModeAction(void)
{
	...
	if ((BMT_status.TOPOFF_charging_time >= MAX_CV_CHARGING_TIME)
	    ||( (BMT_status.bat_vol > (cv_voltage-CV_CHECK_DELAT_FOR_BANDGAP)) &&(charging_full_check() == KAL_TRUE))) {
		BMT_status.bat_charging_state = CHR_BATFULL;
		BMT_status.bat_full = KAL_TRUE;
		g_charging_full_reset_bat_meter = KAL_TRUE;
...
PMU_STATUS BAT_BatteryStatusFailAction(void)
{
	...
	if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] Not reset timer in off charging mode !!\n\r");
	}else{	
	BMT_status.total_charging_time = 0;
	BMT_status.PRE_charging_time = 0;
	BMT_status.CC_charging_time = 0;
