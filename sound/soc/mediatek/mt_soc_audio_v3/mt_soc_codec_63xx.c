...
static int Audio_AmpL_Set_Ftm(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
   mutex_lock(&Ana_Ctrl_Mutex);

   printk("%s()\n ", __func__);

   Ana_Set_Reg(AUDDEC_ANA_CON0, 0xF44F, 0xffff); //Bit[6]: power up right headphone

   mutex_unlock(&Ana_Ctrl_Mutex);
   return 0;
}

static int Audio_AmpR_Set_Ftm(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
   mutex_lock(&Ana_Ctrl_Mutex);

   printk("%s()\n", __func__);

   Ana_Set_Reg(AUDDEC_ANA_CON0, 0xF42F, 0xffff);//Bit[5]: power up left headphone
...
static const struct snd_kcontrol_new mt6331_snd_controls[] =
{
    ...
    SOC_ENUM_EXT("Audio_Amp_Switch_L_HP", Audio_DL_Enum[12], NULL, Audio_AmpL_Set_Ftm),// add this line for HeadsetLR
    SOC_ENUM_EXT("Audio_Amp_Switch_R_HP", Audio_DL_Enum[13], NULL, Audio_AmpR_Set_Ftm),// add this line for HeadsetLR
};
