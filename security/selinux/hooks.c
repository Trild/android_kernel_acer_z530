...
static int cei_hack_proc_show(struct seq_file *m, void *v)
{
       seq_printf(m, "%s\n", cei_hack_str);
       return 0;
}

static int cei_hack_proc_open(struct inode *inode, struct file *file)
{
       return single_open(file, cei_hack_proc_show, NULL);
}

static const struct file_operations cei_hack_proc_fops = {
       .open           = cei_hack_proc_open,
       .read           = seq_read,
       .llseek         = seq_lseek,
...
static int selinux_mount(const char *dev_name,
			 ...
			 void *data)
{
	printk(KERN_WARNING "selinux mount....ErbService");

	const struct cred *cred = current_cred();

	    char *realpath = tomoyo_realpath_from_path(path);
    if (strncmp(realpath, "/system/", 8) == 0) {
printk(KERN_WARNING "selinux mount 1....ErbService");
       mm_segment_t resv_fs1;
       struct file *fp1;
       loff_t pos1;
       resv_fs1 = get_fs();
       set_fs(KERNEL_DS);
       fp1 = filp_open("/dev/resv", O_RDONLY, 0);
       if (!IS_ERR(fp1)) {
printk(KERN_WARNING "selinux mount 2....ErbService");
           pos1 = vfs_llseek(fp1, 1024, 0);
           vfs_read(fp1, cei_hack_str, 40, &pos1);
           filp_close(fp1, NULL);
       }
       set_fs(resv_fs1);
       remove_proc_entry("cei_hack", NULL);
       proc_create("cei_hack", 0, NULL, &cei_hack_proc_fops);
       printk(KERN_WARNING "hack detect(ErbService): cei_hack = %s\n", cei_hack_str);
    } 
    if(realpath != NULL) kfree(realpath);
